...
export const getRecommendedCafes = async (): Promise<Cafe[]> => {
  try {
    const res = await axios.get<{ cafes: Cafe[] }>(`${BASE_URL}/cafes/recommended`);
...
export const getCategories = async (): Promise<Category[]> => {
  try {
    const res = await axios.get<{ categories: Category[] }>(`${BASE_URL}/categories`);
...
export const getCafeReviews = async (cafeId: string): Promise<CafeReview[]> => {
  try {
    const res = await axios.get<{ reviews: CafeReview[] }>(`${BASE_URL}/cafes/${cafeId}/reviews`);
...
export const postCafeReview = async (
  ...
  try {
    const res = await axios.post<{ review: CafeReview }>(`${BASE_URL}/cafes/${cafeId}/reviews`, review);
...
export const getTags = async (category: string): Promise<string[]> => {
  try {
    const res = await axios.get<{ tags: { id: number; category: string; items: string[] }[] }>(`${BASE_URL}/tags`);
